...
const wordReducer = (
  ...
  switch (type) {
    ...
    case SET_WORDLIST:
      const areNotWords = payload.some((word: string) => word.includes(' '));
      var shuffledWordList: string[] = payload.sort(() => Math.random() - 0.5);
      if (areNotWords)
        shuffledWordList = payload.flatMap((token: string) => token.split(' '));
      return {
        ...state,
        typedWord: '',
        typedHistory: [],
